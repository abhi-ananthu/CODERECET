...
(async function indexMongoDBCollection() {
	...
	console.log(mongoUri);
        ...
        console.log(`Found ${documentsToIndex.length} documents with '${contextFieldName}' to index from MongoDB.`);

        if (documentsToIndex.length === 0) {
            console.log("No documents with a valid context field found to index. Skipping upsert.");
            return;
        }
        ...
        for (const doc of documentsToIndex) {
            console.log(`Generating embedding for MongoDB document ID: ${doc.id.substring(0, 10)}...`);
        ...
        console.log(`Generated embeddings for ${vectorsToUpsert.length} documents.`);
        ...
        for (let i = 0; i < vectorsToUpsert.length; i += batchSize) {
            ...
            console.log(`Upserting batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(vectorsToUpsert.length / batchSize)} (${batch.length} vectors)...`);
...
(async function SimiliaritySearch(prompt = "Health care") {
    try {
		...
		console.log(mostRel.metadata);
                ...
                console.log("Match found, but score is below relevance threshold (0.7).");
            }
        } else {
